...
$tariffType = Yii::$app->request->get('tariff');
...
$script = <<<JS
  ...
  function orderNumber() {
    ...
    promise = new Promise((resolve, reject) =>{
        $.ajax({
           url: "$url" + id + "&promocode=" + promocode + "&orderId=" + orderId+"&email="+email+"&source=$source"
         }).done(function(data){
             resolve(data);
         }).fail(function(err){
             alert(err);
...
<div class="prices <?php if (isset($vars->space)) echo '_space'; ?>" name='prices'>
    <div class="container">
        ...
        <div class="prices-items">
            <div class="prices-item "  style="background: transparent;
    border: none;
    box-shadow: none;" >

            </div>
                <?php if (!empty($tariff) && $tariff->day_30 && $tariffType == "1_month"): ?>
